...
function createInstance(_ref) {
  ...
  return function (constructor) {
    ...
    factory.setOptions = function (type, opt) {
      if (shared.isObject(type)) {
        customOptions = _extends({}, customOptions, type);
        typeOtionCache = {};
        return;
      }

      if (shared.isString(type)) {
    ...
    factory.close = function () {
      ...
      if (isSingle && topInstance && shared.isFunction(topInstance.close)) topInstance.close();else {
