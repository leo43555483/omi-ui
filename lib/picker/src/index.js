...
var genUid = shared.getUid();
var DEFAULT_INDEX = 0;

var Picker = function Picker() {
  return {
    name: 'OmiPicker',
    mixins: [provider["default"]('omiPicker')],
    data: function data() {
      return {
        colums: [],
        isSetting: false
    ...
    props: {
      ...
      itemHeight: {
        type: Number,
        default: Colums.DEFAULT_ITEM_HEIGHT
      },
      duration: {
        type: Number,
        default: Colums.DEFAULT_DURATION
      },
      cascade: {
        type: Boolean,
        default: false
      ...
      defaultIndex: {
        ...
        validator: function validator(values) {
          if (!shared.isArray(values)) return true;
          return values.every(function (value) {
            return shared.isNumber(value * 1);
          });
        }
      },
      onConfirm: {
    ...
    methods: {
      ...
      getDefaultIndex: function getDefaultIndex(parent, colum) {
        ...
        var index = shared.isArray(defaultIndex) ? defaultIndex[colum] : defaultIndex;
      ...
      formatCascade: function formatCascade() {
        ...
        while (parent && shared.isArray(parent.children)) {
          ...
          parent = !shared.unDef(children[defaultIndex]) ? children[defaultIndex] : children[DEFAULT_INDEX];
          i += 1;
        }
      ...
      updateCascade: function updateCascade(columIndex) {
        ...
        while (i <= columIndex && shared.isArray(parent.children)) {
      ...
      setValues: function setValues(values, columIndex) {
        ...
        this.$nextTick(function () {
          ...
          } else if (shared.isNumber(columIndex)) {
      ...
      getHeader: function getHeader() {
        ...
        var confirmText = this.confirmText,
            ...
            title = this.title;
        if (shared.unDef(confirmText) && shared.unDef(cancelText) && shared.unDef(title)) return null;
        return h("div", {
    ...
    computed: {
      maskStyles: function maskStyles() {
        ...
        var columHeight = itemHeight * Colums.MAX_VISIBLE_ITEM;
